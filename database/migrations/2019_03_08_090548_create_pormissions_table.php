...
class CreatePormissionsTable extends Migration
{
    /**
     * Run the migrations.
     *权限表
    ...
    public function up()
    {
        Schema::create('pormissions', function (Blueprint $table) {
            $table->increments('id');
            $table->integer('fid')->default(0)->comment('父类id');
            $table->string('name')->comment('权限名字');
    ...
    {
        Schema::dropIfExists('pormissions');
    }
}
