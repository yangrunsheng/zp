...
class RoleController extends Controller
{
     ...
     * 编辑角色权限的页面
     */
    public function rolePormission($roleId)
    {
        $role = new Role();
        $roleP = new RolePormission();

        $assign['role'] = $role->getRoleById($roleId);//获取角色信息

        $assign['pormissions']  = Pormissions::getAllPormissions();//获取所有的权限节点

        //通过用户角色的id获取所分配的所有权限节点id
        $assign['p_ids'] = $roleP->getPormissionByRoleId($assign['role']->id);

        // dd($assign);

        return view('admin.roles.pormissions',$assign);
    }

    /**
     * 保存角色和权限的节点信息
     */
    public function saveRolePormission(Request $request)
    {
        ...
        try{
            ...
            foreach ($params['pormissions'] as $key => $value) {
                $data[$key] = [
